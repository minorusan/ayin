...
function auditProject(root, id) {
  ...
  for (const [alias, c] of g.comps) {
    ...
    for (const leg of entry.legs) {
      ...
      // An OPTIONAL leg is not a missing wire. The HC-05's KEY/EN reads "(optional) → 3.3V or a digital
      // pin, ONLY WHEN entering AT command mode" — the catalog states the optionality in both the name
      // and the destination, and demanding a power wire for it invented a defect out of a documented
      // choice. (Instrument bug 12, same species as the other eleven.)
      if (/\boptional\b|\bonly when\b|\bif you\b|\bnot needed\b/i.test(`${leg.legName} ${leg.connectsTo}`)) continue;
...
// Drop the VALUE that follows `--ladder`, or "2" is taken as a project id named 2 and the run grades
// one non-existent project. A flag's argument is not a positional.
const rawArgs = process.argv.slice(3);
const ladderAt = rawArgs.indexOf('--ladder');
const only = rawArgs.filter((a, i) => !a.startsWith('-') && i !== ladderAt + 1);
