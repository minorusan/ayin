...
Under the Jira comments there is **ask ayin**: a discussion about that ticket. Every message you send starts its **own headless ayin** in the repo the board was served from — it gets the ticket, the earlier turns and your question, searches the codebase, and answers. One message, one run: two questions asked a minute apart are answered in parallel and neither waits for whatever you are doing in the terminal.

Everything the run says arrives in the thread while it works, small and quiet under your question; the answer lands last, larger, and **folds** if the report is long. The thread is markdown at `~/.ayin-cli/sprint/chat/<KEY>.md`, one file per ticket, kept outside your repo so it never lands in a diff or a commit; the run's log sits beside it. It is separate from `post to Jira`: nothing here reaches Jira, so it works even on a read-only credential. While it is working you get a live row under the box with the newest thing the run said and how long it has been going, rather than a spinner that looks the same after four seconds and four minutes. It disappears when the answer lands.
