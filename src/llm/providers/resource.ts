...
/** The wire shape of the `models`/`status` ops. `maradelModel` is the backend's name for the role
...
async function catalogFromStatus(): Promise<ModelCatalog | null> {
  ...
  const shared = s.maradelModel ?? s.sharedModel;
  ...
  if (opts.force || Date.now() >= modelsOpMissingUntil) {
    ...
    if (r && Array.isArray(r.models) && r.models.length > 0 && r.activeModel) {
      ...
      return {
        ...
        sharedModel: r.maradelModel ?? r.sharedModel ?? '',
