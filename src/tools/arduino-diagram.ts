...
export async function runArduinoDiagram(
  ...
  for (const sketch of sketches) {
    ...
    if (grounding.exhausted && pins.length > 0) {
      log('WARN', 'arduino_diagram_grounding_retry', { sketch: sketch.baseName, pins: String(pins.length) });
      grounding = await groundWiring(sketch.baseName, source, readme, pins);
      if (grounding.exhausted) log('ERROR', 'arduino_diagram_grounding_failed_twice', { sketch: sketch.baseName });
    }
...
/**
 * Regenerate the wiring diagram for exactly the sketches `files` touched — the shared entry point for
 * the Presenter executor and the QA executor's `prepare()`. `skip` is how the two stay off each
 * other's toes: whichever runs first reports what it covered, and a single Arduino change never
 * spends its one-LLM-call-per-sketch grounding twice in the same turn. The QA executor additionally
 * puts sketches whose diagram is already newer than the sketch into `skip`, so a multi-pass gate does
 * not redraw an unchanged circuit on every pass.
 */
