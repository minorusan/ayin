...
import { toolLlm, toolLog, toolPrompts, type ToolPrompts } from './runtime.js';

const arduinoPrompts = (): ToolPrompts => toolPrompts('arduino');
...
export async function groundWiring(
  ...
  for (let round = 1; round <= MAX_GROUND_ROUNDS; round++) {
    ...
    try {
      ...
      toolLog().warn('arduino_explain_ground_call_failed', { sketch: sketchName, error: err instanceof Error ? err.message : String(err) });
  ...
  toolLog().warn('arduino_explain_ground_exhausted', { sketch: sketchName, rounds: String(MAX_GROUND_ROUNDS) });
  return { connections: [], exhausted: true };
}
