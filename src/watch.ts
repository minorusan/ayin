...
async function installHook(repo: string): Promise<void> {
  ...
  if (houndInstallAllowed() && ensureHoundHook(repo)) {
 ...
 *
 * A FUNCTION, not a constant: the watch daemon lives for days and the kill switch is thrown from
 * another process, so a value read once at import would keep re-installing the hook the operator
 * just killed — every five minutes, for as long as the daemon runs. `AYIN_WATCH_HOUND=0` stays the
 * per-process opt-out; `ayin kill dog` is the persistent one (see hound-off.ts).
 */
function houndInstallAllowed(): boolean {
  return process.env.AYIN_WATCH_HOUND !== '0' && !isHoundOff();
}
