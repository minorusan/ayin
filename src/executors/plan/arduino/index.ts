...
/** A rendered survey has never carried a blank line; the template's spacing is for the reader of the
 *  .txt, not for the model. Dropping them keeps the extraction byte-identical to the old assembly. */
...
export const arduinoPlanExecutor: PlanExecutor = {
  ...
  survey(ctx: ProjectContext): string {
    ...
    // The prose is `prompts/arduino/survey.txt`; this computes only the measured facts it names.
    // Conditional lines arrive as '' and are dropped with the template's blank lines below — the
    // rendered text has never contained a blank line, and a survey the model reads must not change
    // shape because it moved into a file.
    return dropBlankLines(arduinoPrompts.get('survey', {
      ROOT: ctx.root,
      DETECTED_FROM: `${ctx.evidence}${ctx.greenfield ? ' — NOTHING IS ON DISK YET; this plan creates the project' : ''}`,
      FQBN: fqbn,
      FQBN_SOURCE: source,
      BOARD: board,
      PWM_PINS: pwmPins(board).join(', ') || 'unknown for this board — check its datasheet before using analogWrite',
      SKETCHES: sketches.length
        ? sketches.map((s) => `  - ${s.dir.replace(ctx.root, '.')}/${basename(s.path)} (folder "${basename(s.dir)}")`).join('\n')
        : '  - none yet',
      VIOLATIONS: violations.length ? `  ! SKETCH NAMING VIOLATED: ${violations.join('; ')}` : '',
      LIBRARIES: libs.length ? libs.join(', ') : 'none',
      README_STATE: existsSync(join(ctx.root, 'README.md')) ? 'present' : 'MISSING — ayin creates a stub at project start; fill it in',
      ROOT_ENTRIES: entries.length ? `Existing entries at root: ${entries.join(', ')}` : 'Root is empty.',
    }));
  },
