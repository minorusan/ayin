...
/**
 * `node` IS DELIBERATELY ABSENT, and `config.json` says the same thing.
 *
 * `plan/node` was written for this identical complaint on the other machine and goes further for
 * TypeScript: it writes the package.json, the tsconfig and an entry point that actually starts,
 * deterministically, where this executor's scaffold stops at mkdir + `git init` + README. Selection
 * is "highest priority, ties broken by id", so leaving both claiming `node` at priority 100 would
 * have let an alphabetical accident decide which one bootstraps a project. One owner per type.
 *
...
const config: ExecutorConfig = {
  ...
  description: 'Empty-project planning — the target folder structure, manifest and test convention for a new Python or Unity project, plus git init.',
};
