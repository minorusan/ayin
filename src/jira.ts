/**
 * jira — a thin CONSUMER of the Maradel backend's `jira` resource. ayin no longer calls the Jira REST
 * API itself, no longer reads any Jira credential, and no longer has a `/set`-style auth command —
 * setup/refresh of the actual API token lives on the backend side now (`maradel jiraauth`), because
