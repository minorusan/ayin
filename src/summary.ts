...
export async function updateSummary(goal = ''): Promise<void> {
  ...
  try {
    ...
    // 300 chars per message, and the summarizer is TOLD when a message was cut. Silently clipping its
    // input made it summarize a conversation it could not know was incomplete, and that summary is what
    // `/resume` restores as "context from earlier in this session" — a confident, lossy account with no
    // sign that anything was missing.
    const CLIP = 300;
    const recentText = current.recent
      .map((m) => {
        const body = m.content.length > CLIP
          ? `${m.content.substring(0, CLIP)}… [${m.content.length - CLIP} more chars not shown to you]`
          : m.content;
        return `${m.role}: ${body}`;
      })
      .join('\n');
